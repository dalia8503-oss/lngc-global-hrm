...
# ---------------------------------------------------------
    ...
    with tab2:
        ...
        if st.session_state.workers:
            ...
            with search_col1:
                search_term = st.text_input("사번, 이름, 부서로 검색", placeholder="검색어 입력...")
            
            # 검색 필터링
            if search_term:
                filtered_workers = [w for w in st.session_state.workers 
                                   if search_term in str(w.get('사번', '')) or 
                                      search_term in w.get('이름', '') or 
                                      search_term in w.get('부서', '')]
            else:
                filtered_workers = st.session_state.workers
            
            st.write(f"총 {len(filtered_workers)}명 표시 (전체: {len(st.session_state.workers)}명)")
            st.write("---")
